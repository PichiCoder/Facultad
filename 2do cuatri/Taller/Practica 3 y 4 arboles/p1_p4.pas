...
estructura. }
  ...
  begin
    ...
    if a <> nil then begin
		...
		{Recordar que la logica tiene sentido porque no sabemos que tan menor es lo que entra al if}
		if a^.dato.codigo < unCodigo then
		
			ObtenerCantidadCodigosMenores:=  1 + ObtenerCantidadCodigosMenores(a^.HD, unCodigo) + ObtenerCantidadCodigosMenores(a^.HI, unCodigo)
		
		else ObtenerCantidadCodigosMenores:= ObtenerCantidadCodigosMenores(a^.HI, unCodigo);
		
		end
		
	else ObtenerCantidadCodigosMenores:= 0;
  ...
  begin
    if a <> nil then begin
		...
		if (a^.dato.codigo > codigo1) and (a^.dato.codigo < codigo2) then
			...
			{esto es necesario porque podriamos estar parados en un nodo donde el codigo de producto es menor igual al codigo1 o mayor igual al codigo2
			* y no por eso tenemos que dejar de recorrer el arbol}
			if (a^.dato.codigo <= codigo1) then ObtenerMontoTotalEntreDosCodigos:= ObtenerMontoTotalEntreDosCodigos(a^.HD, codigo1, codigo2)
